...
AIRoute is heading from a monolithic router to a **modular AI platform**: a lightweight
...
| 3.8.53 | `chatCore.ts` decomposition · headless mode (`AIROUTE_HEADLESS=1`) · local candidate build/promote loop |
...
- **`stable/v3`** — the LTS line (3.9.x). Receives fixes, security patches, and provider
  updates. `npm install airoute` (aka `latest`) stays on v3 during the whole v4 cycle.
...
- **`@omniroute/core`** (npm name stays `airoute`) — just the engine: `/v1/*`, routing,
