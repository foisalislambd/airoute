...
adapter the primary transport. Set `AIROUTE_BROWSER_POOL` to one of the same values to
...
`X-AIRoute-Claude-Web-*` headers (`open-sse/executors/claude-web/stream.ts:364`).
...
1. Start AIRoute with `npm run dev` or a built installation.
