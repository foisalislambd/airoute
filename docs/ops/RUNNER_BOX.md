...
```bash
sudo mkdir -p /opt/airoute-ops
sudo cp scripts/ops/runner-janitor.sh /opt/airoute-ops/
sudo chmod +x /opt/airoute-ops/runner-janitor.sh
( sudo crontab -l 2>/dev/null; echo '*/30 * * * * /opt/airoute-ops/runner-janitor.sh >> /var/log/runner-janitor.log 2>&1' ) | sudo crontab -
```
