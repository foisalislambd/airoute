...
1. `npm stage list airoute` — find the stage id (also printed in the workflow summary).
...
`airoute` in stage-only mode so a leaked long-lived token cannot `npm publish`
directly from anywhere — CI can only stage; only the owner's 2FA releases.

**Broken-artifact playbook (unchanged):** `npm deprecate airoute@<bad> "<reason> — use <fixed>"`
