...
| `airoute_ccr_store`               | `write:compression` | Store caller-isolated content in the bounded in-memory CCR store and return a marker plus `ccr://` reference             |
| `airoute_ccr_retrieve`            | `read:compression`  | Retrieve CCR content in full or with head, tail, lines, grep, and stats modes                                            |
| `airoute_ccr_inspect`             | `read:compression`  | Inspect caller-owned CCR metadata without returning content                                                              |
| `airoute_ccr_list`                | `read:compression`  | List paginated metadata for caller-owned CCR blocks                                                                      |
| `airoute_ccr_delete`              | `write:compression` | Delete a caller-owned CCR block                                                                                          |
| `airoute_ccr_stats`               | `read:compression`  | Report caller-scoped memory usage, lifecycle counters, and store limits                                                  |
| `airoute_rtk_discover`            | `read:compression`  | Discover recurring noise in opt-in RTK output samples                                                                    |
| `airoute_rtk_learn`               | `read:compression`  | Generate a reviewable RTK filter draft from opt-in samples                                                               |
...
Separate from the compression tools above, AIRoute includes a post-execution filter that
...
the `_meta` and `AIROUTE_MCP_SCOPES` env fallback — this only populates that first,
...
existing `meta`/env chain unchanged. This does NOT flip `AIROUTE_MCP_ENFORCE_SCOPES`'s
