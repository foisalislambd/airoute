...
per-API-key (`auto_candidate_overrides` table, migration `128`) — AIRoute is
...
[#6940](https://github.com/diegosouzapw/AIRoute/issues/6940). Because combo
...
[#3227/#3233](https://github.com/diegosouzapw/AIRoute/issues/3227) and is
...
| `X-AIRoute-Budget`          | a positive number (max USD per request)                                                                                                                                                 | Hard cost ceiling: candidates whose estimated cost exceeds it are filtered before selection. What happens when **every** candidate exceeds it is controlled by `X-AIRoute-Budget-Fallback` below. |
| `X-AIRoute-Budget-Fallback` | `cheapest` (default, aliases: `cheapest-viable`, `soft`) or `strict` (aliases: `block`, `hard`)                                                                                        | `cheapest`: falls back to the globally cheapest candidate even though it still exceeds the cap (legacy behavior). `strict`: refuses to select — the request fails fast with `HTTP 402` instead of silently overspending. Unknown values are ignored. |
...
curl -sS http://localhost:20128/v1/chat/completions \
  ...
  -H "X-AIRoute-Budget-Fallback: strict" \
