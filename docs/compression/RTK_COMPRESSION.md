...
Within the same scope, RTK TOML schema v1 filters take precedence over AIRoute JSON filters. TOML
...
AIRoute can parse, validate, test, and install declarative filter files using RTK TOML schema v1.
...
AIRoute receives tool output after the client has already captured it, so `filter_stderr = true`
