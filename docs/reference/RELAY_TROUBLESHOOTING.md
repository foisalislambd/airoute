...
serverless backend so AIRoute can egress from a stable region while keeping
the provider API key server-side. This document covers the two failure modes
that operators hit in production and the recovery paths AIRoute ships for
each.
...
Check the relay awareness headers returned by AIRoute's auto-test probe
