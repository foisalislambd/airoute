...
`redirect_uri` with their upstream OAuth app. AIRoute cannot change it — the
...
`localhost` there means **the machine running the browser**, while AIRoute's PKCE
...
- **`20128`** (the dashboard port) makes the origin true-localhost, which is what
  makes AIRoute start the PKCE callback server at all — a LAN origin never
  reaches that branch.
...
| Qwen Code   | `airoute setup-qwen`     | `~/.qwen/settings.json` V4 `modelProviders.openai` entry + `AIROUTE_API_KEY` in `~/.qwen/.env`                                                                     |
