...
**Symptom:** `airoute` / `airoute serve` prints `✔ AIRoute is running!`, but every dashboard or API request returns a bare `500 Internal Server Error`. `~/.airoute/logs/application/app.log` stays empty, `APP_LOG_LEVEL=debug` prints nothing useful, and the response body is plain text (`Internal Server Error`) with no JSON detail.
...
Because the hook never loads, logging never starts — the 500 looks completely undiagnosable. AIRoute creates `~/.cache` (and sets `XDG_CACHE_HOME` when unset) in the CLI entrypoint before Next.js starts so this probe succeeds on Android/Termux.
...
On current AIRoute builds the CLI does this automatically on Android/Termux — a fresh `npx -y airoute@latest` / global install should not require the manual step. If you still see the error after upgrading, create `~/.cache` once as above and restart.
