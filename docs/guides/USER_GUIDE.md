...
1. Subscribe: [Moonshot AI](https://platform.kimi.ai?aff=airoute)
2. Get API key → Dashboard → Add API Key
...
```json
{
  "agents": {
    "defaults": {
      "model": { "primary": "airoute/if/kimi-k2.7-code" }
    }
  },
  "models": {
    "providers": {
...
### Chaining AIRoute Peers

Another AIRoute gateway can be added as a **Custom OpenAI-compatible** provider. Use the
...
```bash
# gateway-a
AIROUTE_INSTANCE_ID=gateway-a
AIROUTE_PEER_URLS=http://gateway-b:20128/v1
AIROUTE_PEER_MAX_HOPS=4
```

```bash
# gateway-b
AIROUTE_INSTANCE_ID=gateway-b
AIROUTE_PEER_URLS=http://gateway-a:20128/v1
AIROUTE_PEER_MAX_HOPS=4
```

Only requests sent to an explicitly allowlisted peer URL receive the
`X-AIRoute-Peer-Trace` header. A gateway rejects a repeated instance ID or exhausted hop
...
into multiple running AIRoute instances.
