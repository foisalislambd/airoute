...
Two AIRoute defaults can silently sabotage multi-hour Codex CLI sessions. Neither is a Codex CLI setting — both live on the AIRoute side. Users migrating a config from upstream proxies that pin accounts and disable idle cutoffs often hit both and conclude AIRoute “cannot sustain a long session.”
...
Related discussions: [#7126](https://github.com/diegosouzapw/AIRoute/discussions/7126) (long task drops), [#5718](https://github.com/diegosouzapw/AIRoute/discussions/5718) (why affinity defaults off). Tracking: [#7287](https://github.com/diegosouzapw/AIRoute/issues/7287).
...
Critical detail: AIRoute’s synthetic SSE **heartbeat does not reset** the idle clock. Only a real upstream body chunk updates `lastChunkTime`. A quiet model that is still “thinking” looks identical to a stalled upstream from the watchdog’s point of view.
...
In the AIRoute process environment (`.env` / compose / systemd):
...
Restart AIRoute after changing these env vars.
...
2. **Raise / disable idle cutoffs** in AIRoute’s environment:
...
3. Keep the usual Codex `config.toml` (`wire_api = "responses"`, correct `base_url`, `AIROUTE_API_KEY`) — no Codex-side affinity/idle knobs exist for these two behaviors.
4. Restart AIRoute, then start the long Codex task.
...
When the idle watchdog fires, AIRoute logs a line shaped like:

```text
[STREAM] Idle timeout: no data from codex for 600000ms (model: cx/gpt-5.5)
```

Grep for `Idle timeout: no data from` (or the code `stream_idle_timeout` / error name `StreamIdleTimeoutError`). The provider segment is whatever AIRoute used for that request (`codex`, another provider id, or `provider` if unknown) — it is not always the literal string `codex`.
...
See [Long-running tasks](#long-running-tasks). Enable session affinity (TTL above task length) and raise or disable `STREAM_IDLE_TIMEOUT_MS` / `FETCH_BODY_TIMEOUT_MS`. Grep AIRoute logs for `Idle timeout: no data from`.
