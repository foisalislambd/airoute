...
`/model` picker normally shows **only** AIRoute's Claude/Anthropic models — a
`kimi/kimi-k2.6` or `glm/glm-5.2` never appears, even though it routes fine.

AIRoute can mirror any enabled model (and combo) under a `claude/…` id so it
passes that filter and shows up in the picker:

```
kimi/kimi-k2.6            →  claude/kimi/kimi-k2.6      "Kimi K2.6 (AIRoute)"
glm/glm-5.2              →  claude/glm/glm-5.2         "GLM 5.2 (AIRoute)"
<combo "custo-otimizado"> →  claude/combo/custo-otimizado
```

When you pick one of these in Claude Code, AIRoute strips the `claude/` wrapper
...
wins), so a plain AIRoute never doubles its catalog for clients that don't use
Claude Code:
...
{
  "env": {
    "ANTHROPIC_BASE_URL": "http://<your AIRoute>:20128",
...
AIRoute refuses to guess. Fix it either way: pin a prefixed id with
...
`AIROUTE_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS=true` (default off;
