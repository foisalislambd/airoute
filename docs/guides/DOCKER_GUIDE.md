...
| `AIROUTE_BASE_PATH`         | URL subpath when the app is published behind a reverse proxy (e.g. `/airoute`)                    | _(empty = root)_         |
| `NEXT_PUBLIC_BASE_URL`        | Public browser origin including the subpath (e.g. `https://host/airoute`)                         | unset                    |
...
Next.js `basePath` is compiled into the standalone bundle. AIRoute records the baked
...
`AIROUTE_BASE_PATH` when the container starts. When they differ and the image was
...
```bash
# .env
AIROUTE_BASE_PATH=/airoute
NEXT_PUBLIC_BASE_URL=https://myhostname.example.com/airoute
```
...
`docker-compose.yml` forwards `AIROUTE_BASE_PATH` as a Docker build-arg and as a
runtime environment variable.

### Pre-built root image + runtime subpath

Published `diegosouzapw/airoute:*` images are built for the domain root. You can still
set `AIROUTE_BASE_PATH` at runtime; the container patches the bundle once on startup.
...
prefix). Traefik should route `PathPrefix(`/airoute`)` to the container without
`StripPrefix`, so Next.js receives `/airoute/...` and serves assets from
`/airoute/_next/...`.
