...
// [model.airoute] ... until the next [section] header or EOF
...
const PREV_DEFAULT_RE = /^# airoute-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
 ...
 * only the flat string fields AIRoute itself writes.
 */
...
const buildModelSection = (model: string, baseUrl: string, apiKey: string): string => {
  const lines = [
    ...
    `name = "AIRoute"`,
    `description = "Routed via AIRoute gateway"`,
...
/** Insert/replace the `[model.airoute]` section, preserving the rest of the file. */
...
const rememberPrevDefault = (toml: string): string => {
  ...
  const marker = `# airoute-prev-default = "${current}"\n`;
...
const hasAIRouteConfig = (modelCfg: GrokModelSection | null): boolean =>
...
// GET — check Grok Build CLI and return current [model.airoute] config
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      hasAIRoute: hasAIRouteConfig(model),
...
// POST — write the [model.airoute] section into ~/.grok/config.toml and set it default
...
// DELETE — remove the [model.airoute] section and restore the previous default
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "AIRoute model slot removed from Grok Build",
    });
  } catch (err) {
    return NextResponse.json({ error: { message: sanitizeErrorMessage(err) } }, { status: 500 });
  }
