...
const getBackupPath = () =>
  path.join(getLocalBackendDir(), "providers", "auth.json.airoute-backup");
...
// ── Check if a base_url points to AIRoute ──────────────────────────────
const isAIRouteUrl = (baseUrl) => {
  if (!baseUrl) return false;
  return baseUrl.includes(":20128") || baseUrl.includes(":3000") || baseUrl.includes("airoute");
};

// ── Check if AIRoute is configured ─────────────────────────────────────
const hasAIRouteConfig = (authFile) => {
  if (!authFile?.providers) return false;
  const provider = authFile.providers[PROVIDER_NAME];
  if (!provider) return false;
  return isAIRouteUrl(provider.base_url);
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Detect if lmstudio is already configured for a non-AIRoute endpoint
    let lmstudioConflict = false;
    if (provider && !isAIRouteUrl(provider.base_url)) {
    ...
    return NextResponse.json({
      installed: true,
      config: authFile,
      hasAIRoute: hasAIRouteConfig(authFile),
      lmstudioConflict,
...
// ── POST - Apply AIRoute as LM Studio provider + switch to local mode ──
/**
 ...
 * up a non-AIRoute provider before it is overwritten. Extracted to keep POST under
 * the complexity gate.
 */
...
> {
  ...
  if (existingProvider && !isAIRouteUrl(existingProvider.base_url) && !overwrite) {
  ...
  if (existingProvider && !isAIRouteUrl(existingProvider.base_url)) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // ── 1-2. Read auth.json, guard non-AIRoute conflicts, back up before overwrite ──
    ...
    // Clean up legacy lc-airoute provider if present
    if (authFile.providers?.["lc-airoute"]) {
      delete authFile.providers["lc-airoute"];
    }
    ...
    return NextResponse.json({
      success: true,
      message: "Settings applied. Restart Letta CLI, then use /model to select a AIRoute model.",
      needsRestart: true,
    });
  } catch (error) {
    return NextResponse.json(
...
// ── DELETE - Remove AIRoute configuration ──────────────────────────────
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const message = restored
      ? "AIRoute config removed. Your original LM Studio provider has been restored. Restart Letta CLI to take effect."
      : "AIRoute config removed. Restart Letta CLI to take effect.";
