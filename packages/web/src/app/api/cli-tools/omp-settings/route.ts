...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      hasAIRoute: !!(ymlProvider || creds.hasAIRoute),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const keyRef = apiKey || "sk_airoute";
    ...
    // 2. Write auth_credentials — so omp sees airoute as "logged in"
    saveOmpCredentials(PROVIDER_ID, keyRef, normalizedBaseUrl);

    return NextResponse.json({
      success: true,
      message:
        "Oh My Pi settings applied! Run omp and all AIRoute models appear under airoute in /model.",
      configPath: getOmpModelsYmlPath(),
    });
  } catch (error) {
    return NextResponse.json(
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "AIRoute removed from Oh My Pi",
    });
  } catch (error) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(error) } },
