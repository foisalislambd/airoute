  ...
  try {
    switch (toolId) {
      ...
      case "opencode":
        ...
        // OpenCode reads ~/.config/opencode/opencode.json — write the AIRoute settings there.
...
async function saveContinueConfig({ baseUrl, apiKey, model }) {
  ...
  // Check if AIRoute entry already exists and update it, or add new
  const existingIdx = models.findIndex(
    (m) =>
      m &&
      (m.airouteManaged === true ||
        normalizeApiBase(m.apiBase) === normalizedBaseUrl.toLowerCase() ||
        normalizeApiBase(m.apiBase).includes("airoute") ||
 ...
 * keys, but make sure the AIRoute provider entry is present and selected.
 */
async function saveHermesConfig({ baseUrl, apiKey, model }) {
  ...
  const nextConfig = {
    ...
    providers: {
      ...
      airoute: {
        ...((existingConfig.providers && existingConfig.providers.airoute) || {}),
        base_url: providerBaseUrl,
        api_key:
          apiKey ||
          (existingConfig.providers &&
            existingConfig.providers.airoute &&
            existingConfig.providers.airoute.api_key) ||
          "",
      },
    },
  };
