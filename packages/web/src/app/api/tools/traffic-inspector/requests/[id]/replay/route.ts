 ...
 *
 * Re-issues the captured request through the local AIRoute instance and
...
const AIROUTE_BASE = process.env.AIROUTE_BASE_URL ?? "http://127.0.0.1:20128";
...
export async function POST(_request: Request, { params }: Params): Promise<Response> {
  ...
  const url = `${AIROUTE_BASE}${entry.path}`;

  const replayHeaders: Record<string, string> = {
    "content-type": "application/json",
    "x-airoute-source": "inspector-replay",
  };
