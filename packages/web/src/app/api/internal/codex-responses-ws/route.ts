...
function getBridgeSecret(): string {
  return process.env.AIROUTE_WS_BRIDGE_SECRET || process.env.OMNIROUTE_WS_BRIDGE_SECRET || "";
}
...
async function prepare(body: JsonRecord) {
  ...
  if (reasoningDecision) {
    ...
    reasoningRouting = isRecord(withDirective._airouteReasoningRouteTrace)
      ? withDirective._airouteReasoningRouteTrace
      : null;
    responseBodyWithMemory = applyReasoningRuleDirective(withDirective) as JsonRecord;
    delete responseBodyWithMemory._airouteReasoningRouteTrace;
  }
...
export async function POST(request: Request) {
  ...
  const receivedSecret =
    request.headers.get("x-airoute-ws-bridge-secret") ||
    request.headers.get("x-omniroute-ws-bridge-secret") ||
    "";
