...
/**
 * Build the `X-AIRoute-Decision` composite header value: `strategy=<name>;
 ...
 * (same alias normalization the `X-AIRoute-Provider` header already applies)
 ...
 */
export function buildAIRouteDecisionHeaderValue({
  strategy = null,
  provider = null,
  latencyMs = 0,
}: {
...
export function buildAIRouteResponseMetaHeaders({
   ...
   * request) surfaced via `X-AIRoute-Decision`. See #6022.
   */
  strategy?: string | null;
  usage?: UsageLike;
  ...
  const decisionValue = buildAIRouteDecisionHeaderValue({ strategy, provider, latencyMs });
  if (decisionValue !== null) {
    headers[AIROUTE_RESPONSE_HEADERS.decision] = decisionValue;
  }
