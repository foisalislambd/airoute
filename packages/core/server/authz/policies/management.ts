...
function requestPeerAddress(ctx: PolicyContext): string | null {
  ...
  const stamped = resolveStampedPeer(
    ...
    process.env.AIROUTE_PEER_STAMP_TOKEN || process.env.OMNIROUTE_PEER_STAMP_TOKEN
  );
...
const WS_BRIDGE_SECRET_HEADER_LEGACY = "x-omniroute-ws-bridge-secret";
...
function isValidWsBridgeRequest(ctx: PolicyContext): boolean {
  ...
  const expected = process.env.AIROUTE_WS_BRIDGE_SECRET || process.env.OMNIROUTE_WS_BRIDGE_SECRET || "";
  if (!expected) return false;
  const provided =
    ctx.request.headers?.get?.(WS_BRIDGE_SECRET_HEADER) ??
    ctx.request.headers?.get?.(WS_BRIDGE_SECRET_HEADER_LEGACY) ??
    "";
  if (!provided) return false;
