...
/** Pre-rename OmniRoute static salt — still decryptable for migrated installs. */
const OMNIROUTE_STATIC_SALT = "omniroute-field-encryption-v1";

let _staticKey: Buffer | null = null;
let _omnirouteStaticKey: Buffer | null = null;
...
/**
 * Derive the pre-rename OmniRoute static-salt key for migrated installs.
 */
function getOmnirouteStaticKey(): Buffer | null {
  if (_omnirouteStaticKey !== null) return _omnirouteStaticKey;

  const secret = process.env.STORAGE_ENCRYPTION_KEY;
  if (!secret || typeof secret !== "string" || secret.trim().length === 0) return null;

  try {
    _omnirouteStaticKey = scryptSync(secret, OMNIROUTE_STATIC_SALT, KEY_LENGTH);
  } catch {
    return null;
  }
  return _omnirouteStaticKey;
}
...
export function decrypt(ciphertext: string | null | undefined): string | null | undefined {
  ...
  try {
    // PRIMARY: Try AIRoute static-salt key first (canonical derivation)
    const decrypted = tryDecryptWithKey(staticKey);
    if (decrypted !== null) {
      return decrypted;
    }

    // OmniRoute rename fallback
    const omniKey = getOmnirouteStaticKey();
    if (omniKey) {
      const omniDecrypted = tryDecryptWithKey(omniKey);
      if (omniDecrypted !== null) {
        return omniDecrypted;
      }
    }

    // Dynamic-salt legacy fallback
