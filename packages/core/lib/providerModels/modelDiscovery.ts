...
// expose (as opposed to the flat `supportedThinkingEfforts` field AIRoute's own
...
// Maps common upstream synonyms onto AIRoute's canonical effort vocabulary
...
export function normalizeDiscoveredModels(models: unknown): SyncedAvailableModel[] {
  ...
  for (const item of items) {
    ...
    deduped.set(id, {
      ...
      ...(() => {
        // #7694/#8347: the flat field (AIRoute's own import format) wins verbatim when
