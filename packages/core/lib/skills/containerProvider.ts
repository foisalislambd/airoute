/**
 * Container runtime providers for the AIRoute skill sandbox.
 *
 * The sandbox historically hardcoded the `docker` CLI. This module abstracts
 * the container runtime so AIRoute can pick the most performant / native
...
const SANDBOX_NAME = (sandboxId: string) => `airoute-${sandboxId}`;
