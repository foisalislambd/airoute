/**
 * Client/server helpers for Next.js `basePath` / `AIROUTE_BASE_PATH` deploys.
 *
 ...
 * Set via next.config `env.NEXT_PUBLIC_AIROUTE_BASE_PATH` from `AIROUTE_BASE_PATH`.
 */
export function getDeployBasePath(
  env: NodeJS.ProcessEnv = typeof process !== "undefined" ? process.env : ({} as NodeJS.ProcessEnv)
): string {
  return normalizeBasePath(
    env.NEXT_PUBLIC_AIROUTE_BASE_PATH || env.AIROUTE_BASE_PATH || ""
  );
}
 ...
 *
 * - Relative absolute paths: `/api/health/ping` → `/airoute/api/health/ping`
 * - Absolute same-origin URLs: `https://host/api/x` → `https://host/airoute/api/x`
