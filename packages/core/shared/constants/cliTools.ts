...
export const CLI_TOOLS: Record<string, CliCatalogEntry> = {
  ...
  qwen: {
    ...
    color: "#10B981",
    description: "Qwen Code CLI — current V4 OpenAI-compatible model provider via AIRoute",
    ...
    notes: [
      {
        type: "info",
        text: "AIRoute is registered under modelProviders.openai using Qwen Code's current bare-array V4 format.",
      },
      {
        type: "info",
        text: "The API key is stored only as AIROUTE_API_KEY in ~/.qwen/.env, leaving your existing provider credentials untouched.",
      },
    ],
    guideSteps: [
...
OPENAI_API_KEY: "{{apiKey}}"`,
  ...
  omp: {
    ...
    description: "Oh My Pi terminal coding agent via AIRoute",
    configType: "custom",
    category: "agent",
    vendor: "OSS",
    acpSpawnable: true,
    ...
    notes: [
      {
        type: "info",
        text: "Oh My Pi reads custom OpenAI-compatible providers from ~/.omp/agent/models.yml. AIRoute adds itself as a provider with auto-discovery — models appear automatically in omp's /model menu.",
      },
      {
        type: "warning",
  ...
  letta: {
    ...
    notes: [
      {
        type: "info",
        text: "Letta CLI uses pi-ai which sends OpenAI-compatible requests. AIRoute configures it as an OpenAI provider with custom base URL.",
      },
      {
        type: "info",
        text: "CLI (Local Mode): AIRoute auto-configures ~/.letta/lc-local-backend/providers/auth.json. Use 'letta --info' to check if local mode is enabled.",
      },
      {
        type: "warning",
