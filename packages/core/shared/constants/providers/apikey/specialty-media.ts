...
export const APIKEY_PROVIDERS_SPECIALTY = {
  ...
  segmind: {
    ...
    authHint:
      "Use your Segmind API key in the x-api-key header. AIRoute targets https://api.segmind.com/v1/<model> and returns the generated image/video bytes directly.",
    apiHint:
      "Segmind exposes 200+ hosted image and video models (Flux, SDXL, SD3, Kandinsky, Wan, Hunyuan, LTX, Kling, ...) under a single POST /v1/<model> REST call per model. AIRoute ships a curated starter subset for each modality; more models can be routed by using their Segmind slug as the model id.",
    hasFree: true,
