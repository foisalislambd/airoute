...
describe("useDisplayBaseUrl", () => {
  ...
  it("appends AIROUTE basePath when resolving a public browser origin", () => {
    expect(
      resolveDisplayBaseUrl(
  ...
  it("derives basePath from NEXT_PUBLIC_BASE_URL when env basePath is unset", () => {
    expect(
      resolveDisplayBaseUrl("https://api.example.com/airoute", "https://api.example.com", "/")
    ).toBe("https://api.example.com/airoute");
  });

  it("normalizes basePath values without a leading slash", () => {
    expect(normalizeBasePath("airoute")).toBe("/airoute");
    expect(normalizeBasePath("/airoute/")).toBe("/airoute");
