...
/**
 * Resolve the deploy basePath for display URLs (e.g. `/airoute`).
 *
 * Priority:
 * 1. Explicit `envBasePath` argument / `NEXT_PUBLIC_AIROUTE_BASE_PATH`
  ...
  const fromEnv = normalizeBasePath(
    envBasePath ??
      (typeof process !== "undefined"
        ? process.env.NEXT_PUBLIC_AIROUTE_BASE_PATH || process.env.AIROUTE_BASE_PATH
        : undefined)
  );
 ...
 * @param envBasePath       `NEXT_PUBLIC_AIROUTE_BASE_PATH` / `AIROUTE_BASE_PATH`
 */
export function resolveDisplayBaseUrl(
  ...
  // Example: NEXT_PUBLIC_BASE_URL=https://host/airoute must not be collapsed to
  ...
  // reverse-proxy subpaths (AIROUTE_BASE_PATH).
 ...
 *   1. Public browser origin (+ AIROUTE_BASE_PATH when set) — proves the
