...
export const AIROUTE_PEER_TRACE_HEADER = "X-AIRoute-Peer-Trace";

type HeaderSource = Headers | Record<string, unknown> | null | undefined;
type PeerEnvironment = {
  AIROUTE_INSTANCE_ID?: string;
  AIROUTE_PEER_URLS?: string;
  AIROUTE_PEER_MAX_HOPS?: string;
};
...
export function isConfiguredAIRoutePeer(
  targetUrl: string,
...
export function inspectPeerRequest(
  ...
  const trace = parsePeerTrace(readHeader(headers, AIROUTE_PEER_TRACE_HEADER));
  if (trace.includes(instanceId)) {
    return {
      code: "peer_loop_detected",
      message: "AIRoute peer routing loop detected",
    };
  }
  if (trace.length >= getMaxPeerHops(env)) {
    return {
      code: "peer_hop_limit_exceeded",
      message: "AIRoute peer routing hop limit exceeded",
    };
  }
  return null;
}
...
/**
 * Append this instance to the peer trace for an explicitly allowlisted AIRoute URL.
...
export function applyPeerTraceHeader(
  ...
  const instanceId = getInstanceId(env);
  if (!instanceId || !isConfiguredAIRoutePeer(targetUrl, env)) return false;

  const trace = parsePeerTrace(readHeader(clientHeaders, AIROUTE_PEER_TRACE_HEADER));
  if (!trace.includes(instanceId)) trace.push(instanceId);
  const traceHeaderLower = AIROUTE_PEER_TRACE_HEADER.toLowerCase();
  for (const key of Object.keys(outgoingHeaders)) {
    if (key.toLowerCase() === traceHeaderLower) delete outgoingHeaders[key];
  }
  outgoingHeaders[AIROUTE_PEER_TRACE_HEADER] = trace.join(",");
  return true;
}
