...
export const QWEN_CODE_ENV_KEY = "AIROUTE_API_KEY";
...
/**
 * Identifies only entries owned by AIRoute. Generic custom endpoints are not
...
export const isManagedQwenCodeModel = (value: unknown): boolean => {
  ...
  if (value.envKey === QWEN_CODE_ENV_KEY || value.id === "airoute") return true;

  return (
    typeof value.name === "string" &&
    value.name.endsWith(" (AIRoute)") &&
...
export const buildQwenCodeModel = ({
  ...
  return {
    id: normalizedModel,
    name: `${String(modelName || normalizedModel).trim()} (AIRoute)`,
...
/**
 * Merge one AIRoute model into Qwen Code's current V4 settings contract.
...
export const mergeQwenCodeSettings = (
  ...
  if (next.selectedProvider === "airoute") delete next.selectedProvider;

  return next;
};

export const findAIRouteQwenCodeModel = (settings: unknown): JsonRecord | undefined => {
...
export const hasAIRouteQwenCodeConfig = (settings: unknown): boolean =>
  findAIRouteQwenCodeModel(settings) !== undefined;

/** Remove only AIRoute-owned Qwen Code entries and selection state. */
...
const OWNED_ENV_LINE = /^\s*(?:export\s+)?AIROUTE_API_KEY\s*=/;
...
export const mergeQwenCodeEnv = (existing: unknown, apiKey: unknown): string => {
  ...
  lines.push(`${QWEN_CODE_ENV_KEY}=${JSON.stringify(String(apiKey || "sk_airoute"))}`);
  return `${lines.join("\n")}\n`;
};
