...
} from "../../../core/lib/db/compression.ts";
import { getCompressionAnalyticsSummary } from "../../../core/lib/db/compressionAnalytics.ts";
import { getCacheStatsSummary } from "../../../core/lib/db/compressionCacheStats.ts";
import { listCompressionCombos } from "../../../core/lib/db/compressionCombos.ts";
...
export async function handleCcrStoreTool(
  ...
  await logToolCall("airoute_ccr_store", auditInput, output, Date.now() - start, true);
  return output;
}
...
export async function handleCcrRetrieveTool(
  ...
  if ((!args.mode || args.mode === "full") && metadata.bytes > MAX_CCR_MCP_FULL_BYTES) {
    ...
    await logToolCall("airoute_ccr_retrieve", args, output, Date.now() - start, true);
    return output;
  }
...
export async function handleCcrInspectTool(
  ...
  await logToolCall("airoute_ccr_inspect", args, output, Date.now() - start, Boolean(metadata));
  return output;
}
...
export async function handleCcrListTool(
  ...
  await logToolCall("airoute_ccr_list", args, output, Date.now() - start, true);
  return output;
}
...
export async function handleCcrDeleteTool(
  ...
  await logToolCall("airoute_ccr_delete", args, output, Date.now() - start, true);
  return output;
}
...
export async function handleCcrStatsTool(
  ...
  await logToolCall("airoute_ccr_stats", args, output, Date.now() - start, true);
  return output;
}
