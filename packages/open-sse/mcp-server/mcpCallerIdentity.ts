 ...
 *
 * On stdio transport (`airoute --mcp`) there is no HTTP context, so we fall
 * back to the AIROUTE_API_KEY / ROUTER_API_KEY environment variable.
 *
...
import { extractApiKey } from "../../core/sse/services/auth.ts";
import { getApiKeyMetadata } from "../../core/lib/db/apiKeys.ts";
 ...
 *
 * Falls back to AIROUTE_API_KEY / ROUTER_API_KEY env vars for stdio transport
...
/**
 * Resolve the principal id from the AIROUTE_API_KEY (or ROUTER_API_KEY) env var.
...
async function resolvePrincipalFromEnv(): Promise<string | undefined> {
  const rawKey = process.env.AIROUTE_API_KEY || process.env.ROUTER_API_KEY;
  if (!rawKey) return undefined;
  try {
