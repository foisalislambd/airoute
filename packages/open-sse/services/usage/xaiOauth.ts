 ...
 * with the connection's OAuth access token, falling back to AIRoute's own
...
export async function getXaiOauthUsage(
  ...
  try {
    ...
    const used =
      ...
      0;
    return {
      plan: "xAI OAuth (Grok) · AIRoute-tracked",
      message: "Live weekly quota unavailable; showing AIRoute-routed token totals only.",
      quotas: {
        monthly: {
          used,
          total: 0,
