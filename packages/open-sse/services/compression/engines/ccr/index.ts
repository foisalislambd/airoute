 ...
 * ENTIRE prompt for any caller that cannot resolve `airoute_ccr_retrieve`
...
function processMessages(
  ...
  const result = messages.map((msg) => {
    ...
    // AIRoute is used as a chat-completion PROVIDER, the upstream LLM has no
    // way to call `airoute_ccr_retrieve` and expand markers — replacing tool
...
export const ccrEngine: CompressionEngine = {
  ...
  apply(body: Record<string, unknown>, options?: CompressionEngineApplyOptions): CompressionResult {
    ...
    // #8033: teach MCP-capable callers the marker → airoute_ccr_retrieve contract
