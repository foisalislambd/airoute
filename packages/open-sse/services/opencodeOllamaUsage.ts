...
// AIROUTE_OPENCODE_GO_QUOTA_URL explicitly. Never hardcode a third-party
// host here (a previous default silently sent the user's API key to an
// unrelated Z.AI endpoint).
const OPENCODE_GO_QUOTA_URL = process.env.AIROUTE_OPENCODE_GO_QUOTA_URL?.trim() || "";
  ...
  if (!OPENCODE_GO_QUOTA_URL) {
    return {
      message:
        ...
        "or set AIROUTE_OPENCODE_GO_QUOTA_URL to opt in to an explicit quota endpoint.",
    };
  }
      ...
      if (res.status === 401 || res.status === 403) {
        return {
          message:
            ...
            "AIROUTE_OPENCODE_GO_QUOTA_URL endpoint. " +
            "Set OPENCODE_GO_WORKSPACE_ID and OPENCODE_GO_AUTH_COOKIE to enable dashboard quota scraping.",
        };
      }
      return {
