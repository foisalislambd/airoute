 ...
 *      requests (quotaShareInflight) wins; ties keep the DRR order. Ordering
 *      does not reserve a slot (the production caller cannot release it).
 *      Dispatch concurrency is a separate semaphore
 *      (`acquireQuotaShareConcurrencySlot`), not this inflight map.
 *
...
import { isBucketSaturated } from "../../../core/lib/quota/accountBuckets.ts";
...
export function selectQuotaShareTarget(
  ...
  // Do NOT reserve an in-flight slot at ordering time. The only production
  // caller (`applyStrategyOrdering`) discards `decrementInflight`, which left
  // phantom load for the full lease (~120s) and demoted healthy accounts.
  // Dispatch-time concurrency uses a separate semaphore
  // (`acquireQuotaShareConcurrencySlot`); it does not feed `getInflight()`.
  // P2C therefore only reflects inflight recorded by other writers.
  ...
  return { target: winner, orderedTargets, decrementInflight: noOp };
}
