...
import type { ResilienceSettings } from "../../../core/lib/resilience/settings";
...
export interface ResolveAutoStrategyDeps {
  ...
  relayOptions?: {
    ...
    /** Per-request X-AIRoute-Budget-Fallback value ("cheapest" | "strict") — #3470. */
...
export async function resolveAutoStrategyOrder(
  ...
  // Per-request overrides (#6023 / #6024 / #6025 / #3470): X-AIRoute-Budget,
  // X-AIRoute-Budget-Fallback and X-AIRoute-Mode headers (threaded via
  ...
  // per-request X-AIRoute-Mode override — but `scoreAutoTargets()` (the fallback
  ...
  try {
    const { getLKGP } = await import("../../../core/lib/localDb");
