 ...
 *   X-AIRoute-Budget-Fallback: cheapest | strict                                 (#3470)
 *
...
/**
 * Parse the `X-AIRoute-Budget-Fallback` header into a budget-fallback policy override.
...
export function resolveRequestAutoControls(headers: {
  ...
  const modeHeader = headers.get("x-airoute-mode")?.trim() || null;
  const budgetHeader = headers.get("x-airoute-budget")?.trim() || null;
  const budgetFallbackHeader = headers.get("x-airoute-budget-fallback")?.trim() || null;
