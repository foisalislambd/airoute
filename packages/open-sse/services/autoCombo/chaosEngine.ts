 ...
 * not reliable here because AIRoute may force a streaming envelope internally.
 */
 ...
 *
 * Supports BOTH wire formats AIRoute may emit:
 ...
 * `x-airoute-chaos-error` header and an error final chunk (status stays 200
...
export async function handleChaosChat(opts: {
  ...
  return new Response(stream, {
    ...
    headers: {
      ...
      "X-AIRoute-Chaos": "true",
      "X-AIRoute-Chaos-Panel": String(panel.length),
      "X-AIRoute-Chaos-Primary": primaryModel ?? "",
    },
  });
}
