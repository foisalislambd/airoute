...
export async function createVirtualAutoCombo(
  ...
  // operators can override via env var AIROUTE_CHAOS_MAX_PANEL (default 5).
  //
  ...
  const CHAOS_MAX_PANEL = (() => {
    const env = process.env.AIROUTE_CHAOS_MAX_PANEL;
      ...
      ...(isChaos
        ? {
            chaos: {
              ...
              tuning: {
                panelHardTimeoutMs:
                  Number(process.env.AIROUTE_CHAOS_PANEL_TIMEOUT_MS) || undefined,
                minPanel: Number(process.env.AIROUTE_CHAOS_MIN_PANEL) || undefined,
              },
            },
          }
        : {}),
