...
} from "../../core/lib/resilience/settings";
import {
  STANDARD_HEADERS,
  ANTHROPIC_HEADERS,
  parseResetTime,
 ...
 */
function limiterKeyBelongsToConnection(key, connectionId) {
  if (!connectionId || typeof key !== "string") return false;
  // Keys are `${provider}:${connectionId}` or `${provider}:${connectionId}:${scope}`.
  // Codex uses `${provider}:${connectionId}:${scope}` via getCodexRateLimitKey.
  const colon = key.indexOf(":");
  if (colon < 0) return false;
  const rest = key.slice(colon + 1);
  return rest === connectionId || rest.startsWith(`${connectionId}:`);
}
...
export function disableRateLimitProtection(connectionId) {
  ...
  for (const [key, limiter] of Array.from(limiters)) {
    if (limiterKeyBelongsToConnection(key, connectionId)) {
      limiters.delete(key);
      lastDispatchAt.delete(key);
      limiterLastUsed.delete(key);
      trackAsyncOperation(limiter.disconnect());
...
export async function withRateLimit(
  ...
  try {
    ...
    // case — a clear, AIRoute-owned, classifiable error — so combo's transient-error
    ...
    if (err?.message === "rate-limit-watchdog-wedge-reset") {
      const wedgeErr = new Error(
        `Request dropped: the local rate-limit queue for ${model ? `${provider}/${model}` : provider} ` +
          `was detected as wedged (stalled with nothing executing) and force-reset. This is AIRoute's ` +
...
export function updateFromHeaders(provider, connectionId, headers, status, model = null) {
  ...
  if (status === 429) {
    ...
    // Evict the spent limiter, then install a fresh one with reservoir=0 so
    // header-only Retry-After is honored until the pause elapses (body-based
    // updateFromResponseBody already did this; header path previously dropped
    // the pause entirely after disconnect).
    limiters.delete(limiterKey);
    lastDispatchAt.delete(limiterKey);
    limiterLastUsed.delete(limiterKey);
    trackAsyncOperation(limiter.disconnect());

    const paused = getLimiter(provider, connectionId, model);
    paused.updateSettings({
      reservoir: 0,
