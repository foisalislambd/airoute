...
import {
  ...
  getLastSessionModel,
  getHandoff,
} from "../../core/lib/db/contextHandoffs.ts";
import { extractSessionAffinityKey } from "@/sse/services/auth";
import { getHiddenModelsByProvider } from "@/models";
import { resolveModelLockoutSettings } from "../../core/lib/resilience/modelLockoutSettings";
import { fetchCodexQuota } from "./codexQuotaFetcher.ts";
import { evaluateQuotaCutoff, getQuotaFetcher, type QuotaInfo } from "./quotaPreflight.ts";
import * as semaphore from "./rateLimitSemaphore.ts";
import { getCircuitBreaker } from "../../core/shared/utils/circuitBreaker";
import { parseModel } from "./model.ts";
import { createComboContext } from "./combo/context.ts";
import { phaseComboSetup } from "./combo/comboSetup.ts";
import { checkCredentialGate, logCredentialSkip } from "./credentialGate.ts";
import { emit } from "../../core/lib/events/eventBus";
import { notifyWebhookEvent } from "../../core/lib/webhookDispatcher";
...
import { getCachedProviderConnections } from "../../core/lib/db/readCache";
import {
...
} from "../../core/sse/services/cooldownAwareRetry.ts";
      ...
      const executeTarget = async (
        ...
        for (let retry = 0; retry <= maxRetries; retry++) {
          ...
          if (result.ok) {
            ...
            if (_sticky.messageHash) {
              const stickyConn = effectiveConnectionId || target.connectionId;
              if (stickyConn) recordStickyBinding(_sticky.messageHash, stickyConn); // LKGP (#919):
            }
            if (provider) {
              const connId = effectiveConnectionId || undefined;
              void (async () => {
                try {
                  const { setLKGP } = await import("../../core/lib/localDb");
                  await Promise.all([
          ...
          try {
            ...
            if (text) {
              ...
              retryAfter =
                ...
                typeof parsedError?.reset_seconds === "number" &&
                parsedError.reset_seconds > 0
                  ? new Date(Date.now() + parsedError.reset_seconds * 1000).toISOString()
                  : null);
            }
          } catch {
            /* Clone parse failed */
