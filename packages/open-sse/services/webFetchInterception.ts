 ...
 * synthetic `airoute_web_fetch` function tool; the actual `/v1/web/fetch` call
...
export const AIROUTE_WEB_FETCH_FALLBACK_TOOL_NAME = "airoute_web_fetch";
...
function buildFallbackTool(targetFormat?: string | null): JsonRecord {
  const name = AIROUTE_WEB_FETCH_FALLBACK_TOOL_NAME;
  const description = [
...
export function prepareWebFetchFallbackBody<T extends WebFetchFallbackBody>(
  ...
  if (!toolNames.has(AIROUTE_WEB_FETCH_FALLBACK_TOOL_NAME)) {
  ...
  if (isBuiltInWebFetchToolChoice(body.tool_choice)) {
    nextBody.tool_choice = (
      isResponsesTarget
        ? { type: "function", name: AIROUTE_WEB_FETCH_FALLBACK_TOOL_NAME }
        : { type: "function", function: { name: AIROUTE_WEB_FETCH_FALLBACK_TOOL_NAME } }
    ) as T["tool_choice"];
  }
  ...
  return {
    ...
    fallback: {
      enabled: true,
      toolName: AIROUTE_WEB_FETCH_FALLBACK_TOOL_NAME,
