...
export async function handleChatCore({
  ...
  if (webFetchFallbackPlan.enabled) {
    ...
    log?.info?.(
      "TOOLS",
      `Converted ${webFetchFallbackPlan.convertedToolCount} web_fetch tool(s) to AIRoute fallback for ${provider}`
    );
  }
  ...
  const reasoningRouteDecision =
    body && typeof body === "object"
      ? (body as Record<string, unknown>)._airouteReasoningRouteTrace
      : null;
  if (reasoningRouteDecision) {
    reqLogger.logRouteDecision(reasoningRouteDecision);
    body = { ...(body as Record<string, unknown>) };
    delete (body as Record<string, unknown>)._airouteReasoningRouteTrace;
  }
  ...
  if (body && Array.isArray(allMessages) && allMessages.length > 0) {
    ...
    try {
      ...
      if ((isCombo && comboName) || routingComboId) {
        try {
          const { getComboByName } = await import("../../core/lib/localDb");
          ...
          if (routingComboIds.length > 0) {
            const { getCompressionComboForRoutingCombo } =
              await import("../../core/lib/db/compressionCombos.ts");
      ...
      try {
        const { listCompressionCombos } = await import("../../core/lib/db/compressionCombos.ts");
