 ...
 * This budget includes each header name, separator, value, and trailing CRLF. AIRoute's own
...
function isAIRouteInternalHeader(headerName: string): boolean {
  return headerName.toLowerCase().startsWith("x-airoute-");
}
 ...
 *
 * If AIRoute re-emits those headers from an App Router route handler, Next
...
export function buildStreamingResponseHeaders(
  ...
  providerHeaders.forEach((value, key) => {
    ...
    if (
      ...
      isAIRouteInternalHeader(normalized)
    ) {
      return;
    }
    candidates.push({
