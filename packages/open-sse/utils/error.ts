 ...
 * `x-airoute-combo-*` headers and a `diagnostics` field in the OpenAI-shaped
 ...
 * carries a `recovery` hint it is mirrored as `x-airoute-recovery-action` /
 * `x-airoute-recovery-next-step` / `x-airoute-retry-after-seconds` headers and as a
...
export function errorResponseWithComboDiagnostics(
  ...
  const headers: Record<string, string> = {
    ...
    "x-airoute-combo-pool-size": String(safe.poolSize),
    "x-airoute-combo-attempted": String(safe.attempted),
    "x-airoute-combo-excluded": excludedHeader,
    "x-airoute-combo-terminal-reason": toHeaderSafeAscii(safe.terminalReason.slice(0, 200)),
  };

  if (safe.recovery) {
    headers["x-airoute-recovery-action"] = safe.recovery.action;
    ...
    headers["x-airoute-recovery-next-step"] = toHeaderSafeAscii(safe.recovery.next_step).slice(
      0,
      128
    );
    if (
      ...
      headers["x-airoute-retry-after-seconds"] = String(safe.recovery.retry_after_seconds);
    }
  }
