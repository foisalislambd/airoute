 ...
 *
 * Problem (#7791): when AIRoute is installed via `npm i -g airoute`, the
 * package files live under `node_modules/airoute/`. tsx's tsconfig-path
 ...
 *   - `@/*`             → `./packages/core/*`
 *   - `@omniroute/open-sse`    → `./packages/open-sse/index.ts`
 *   - `@omniroute/open-sse/*`  → `./packages/open-sse/*`
...
/**
 * Alias mapping table — mirrors packages/web/tsconfig.json `paths` (repo layout).
 * Processed top-to-bottom; first matching prefix wins.
 *
 * Each entry:
 *   prefix  — specifier prefix to match (e.g. `"@/"`, `"@omniroute/open-sse/"`)
 *   target  — directory name under the package root (e.g. `"packages/core"`)
 ...
 *             `<root>/packages/open-sse/index.ts`).
 *
 ...
 *   "@/..."                     → <root>/packages/core/...
 *   "@omniroute/open-sse/..."   → <root>/packages/open-sse/...
 *   "@omniroute/open-sse"       → <root>/packages/open-sse/index.*
 *
...
export async function registerAliasResolver(root) {
  ...
  // Require the monorepo packages/core tree (former root src/) before installing
  // a hook that would otherwise rewrite to nowhere.
