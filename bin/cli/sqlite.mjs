...
// prebuilt addon, etc.), every `airoute doctor` DB check reported a false FAIL
...
async function openWithSyncDriverFallback(dbPath, options, importError) {
  try {
    const { tryOpenSync } = await import("../../packages/core/lib/db/adapters/driverFactory.ts");
