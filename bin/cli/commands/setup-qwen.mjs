/** Configure Qwen Code's OpenAI-compatible provider for AIRoute. */
...
} from "../../../packages/core/shared/services/qwenCodeConfig.ts";
...
export function resolveQwenTarget(opts = {}) {
  ...
  if (!root || !(opts.apiKey ?? opts["api-key"])) {
    try {
      context = resolveActiveContext(opts.context ?? process.env.AIROUTE_CONTEXT);
    } catch {
...
export async function runSetupQwenCommand(opts = {}) {
  ...
  try {
    ...
    if (dryRun) {
      ...
      printInfo(`[dry-run] credential → ${envPath} (AIROUTE_API_KEY)`);
      return 0;
    }
    ...
    printSuccess(`Updated ${envPath} (AIROUTE_API_KEY only)`);
...
export function registerSetupQwen(program) {
  program
    .command("setup-qwen")
    .description("Configure Qwen Code's upstream V4 modelProviders format for AIRoute")
    .option("--port <port>", "Local AIRoute port (ignored when --remote is set)", "20128")
    .option("--remote <url>", "Remote AIRoute URL")
    .option("--api-key <key>", "AIRoute API key")
