...
function readCache() {
  try {
    ...
    if (process.env.AIROUTE_DEBUG_COMPLETION) {
      console.error("[airoute completion] readCache failed:", err?.message ?? err);
    }
  }
  return null;
}
...
async function refreshCache(opts = {}) {
  ...
  try {
    ...
    if (process.env.AIROUTE_DEBUG_COMPLETION) {
      console.error("[airoute completion] refreshCache failed:", err?.message ?? err);
    }
  }
  ...
  try {
    ...
    if (process.env.AIROUTE_DEBUG_COMPLETION) {
      console.error("[airoute completion] writeCache failed:", err?.message ?? err);
    }
  }
  return data;
}
