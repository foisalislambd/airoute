...
export class ServerSupervisor {
  ...
  start() {
    ...
    // #6321: stdout used to be discarded (`"ignore"`) whenever `--log`/AIROUTE_SHOW_LOG
  ...
  async handleExit(code, err) {
    ...
    if (this.restartCount >= this.maxRestarts) {
      ...
      if (this.onCrashCallback) {
        const action = await this.onCrashCallback(this.crashLog);
