...
// ahead of expensive self-heal hooks), adapted to AIRoute's Commander CLI where the
...
await import("../packages/open-sse/utils/setupPolyfill.ts");

// #7791: tsx's tsconfig-path resolution does not apply when AIRoute is
// installed globally (files live under node_modules/airoute/), so bare
// `@/...` specifiers (declared in tsconfig.json paths as `@/* → ./packages/core/*`)
// fail with ERR_MODULE_NOT_FOUND. Register an ESM resolve hook that maps
// `@/...` to absolute file URLs under <ROOT>/packages/core/. Safe no-op in
// environments where module.register is unavailable.
...
function loadEnvFile() {
  ...
  if (process.env.AIROUTE_CLI_SKIP_REPO_ENV !== "1") {
...
// Generate STORAGE_ENCRYPTION_KEY if not set (persisted to ~/.airoute/.env)
...
// key or write ~/.airoute/.env — running a read-only command should never
// mutate the data dir.
if (shouldProvisionStorageKey(process.argv)) {
  ...
  if (!process.env.STORAGE_ENCRYPTION_KEY) {
    ...
    // Writing only to ~/.airoute (the container home, not a volume) silently lost the key on
    // container recreation, leaving the persisted encrypted DB undecryptable (regression of #1622).
    const dataDir = process.env.DATA_DIR || join(homedir(), ".airoute");
    const envPath = join(dataDir, ".env");
...
// Apply --lang before Commander parses (program descriptions call t() during setup)
{
  ...
  const langEnv = process.env.AIROUTE_LANG;
...
process.on("exit", () => {
  if (process.env.AIROUTE_NO_UPDATE_NOTIFIER) return;
  ...
  if (_notifier.update) {
    _notifier.notify({
      ...
      message:
        ...
        "Run `npm install -g airoute` or `airoute update --apply`",
    });
  }
});
