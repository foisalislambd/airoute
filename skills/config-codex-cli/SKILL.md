...
Two AIRoute defaults silently break multi-hour Codex sessions. Document them whenever configuring Codex for overnight / multi-hour work. Full guide: `docs/guides/CODEX-CLI-CONFIGURATION.md` → **Long-running tasks**.
...
- Codex session keys (`x-codex-session-id`, `x-session-id`, `x-airoute-session`, body `prompt_cache_key` / `session_id`) are only used for pinning when TTL > 0.
...
3. Restart AIRoute. Leave Codex `config.toml` as usual (`wire_api = "responses"`, correct `base_url`).
