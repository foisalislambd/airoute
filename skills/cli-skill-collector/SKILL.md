...
description: "Detect installed CLI coding tools (Claude Code, Codex, Cursor, Copilot, Cline and more), search GitHub for matching agent skills, and install them to the detected tools via AIRoute's built-in APIs."
---
...
Discover and install agent skills for your coding CLI tools — all through AIRoute's built-in APIs.
...
No separate Skill Collector app needed — AIRoute's own CLI detection + GitHub search handles everything.

---

## Step 1 — Detect installed coding tools

Query AIRoute's CLI tool detection to find which coding agents are installed:

```bash
curl -H "Authorization: Bearer $AIROUTE_API_KEY" http://localhost:20128/api/skills/collect/detect
```

This returns:

- Every CLI tool in AIRoute's catalog (`CLI_TOOL_IDS`: claude, codex, cursor, copilot, opencode, cline, kilocode, hermes, hermes-agent, openclaw, droid, continue, qwen, windsurf, devin, antigravity, etc.)
...
```bash
curl -H "Authorization: Bearer $AIROUTE_API_KEY" \
...
curl -X POST http://localhost:20128/api/skills/collect/install \
  -H "Authorization: Bearer $AIROUTE_API_KEY" \
...
```bash
AUTH_HEADER="Authorization: Bearer $AIROUTE_API_KEY"
...
- AIRoute must be running locally on port 20128 (default) — see `docs/frameworks/SKILLS.md` for custom-port setups.
...
- This replaces the standalone Skill Collector Python app — all logic is now inside AIRoute.
<!-- skill:custom-end -->
