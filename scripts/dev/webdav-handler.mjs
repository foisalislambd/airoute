...
// ─────────────────────────────────────────────────────────────────────────────
// Encryption: minimal port of packages/core/lib/db/encryption.ts
// Must reproduce the EXACT format: enc:v1:<iv_hex>:<ciphertext_hex>:<authTag_hex>
// Key derivation: primary airoute-field-encryption-v1, fallback omniroute-field-encryption-v1
// ─────────────────────────────────────────────────────────────────────────────

const ENC_PREFIX = "enc:v1:";
const STATIC_SALT = "airoute-field-encryption-v1";
const OMNIROUTE_STATIC_SALT = "omniroute-field-encryption-v1";
const KEY_LENGTH = 32;
const AUTH_TAG_LENGTH = 16;
const ALGORITHM = "aes-256-gcm";

let _cachedStaticKey = null;
let _cachedOmnirouteKey = null;
...
function getOmnirouteStaticKey() {
  if (_cachedOmnirouteKey !== null) return _cachedOmnirouteKey;
  const secret = process.env.STORAGE_ENCRYPTION_KEY;
  if (!secret || typeof secret !== "string" || secret.trim().length === 0) return null;
  try {
    _cachedOmnirouteKey = scryptSync(secret, OMNIROUTE_STATIC_SALT, KEY_LENGTH);
  } catch {
    return null;
  }
  return _cachedOmnirouteKey;
}

/**
 * Decrypt a value that may be encrypted with the AIRoute/OmniRoute enc:v1: format.
...
export function decryptStored(stored) {
  ...
  return tryWithKey(getStaticKey()) ?? tryWithKey(getOmnirouteStaticKey());
}
...
function getDefaultDataDir() {
  ...
  const airouteDir = path.join(homeDir, ".airoute");
  const omnirouteDir = path.join(homeDir, ".omniroute");

  const preferExisting = (airoutePath, omniPath) => {
    try {
      if (fs.existsSync(airoutePath) && fs.statSync(airoutePath).isDirectory()) return airoutePath;
    } catch {
      /* ignore */
    }
    try {
      if (fs.existsSync(omniPath) && fs.statSync(omniPath).isDirectory()) return omniPath;
    } catch {
      /* ignore */
    }
    return airoutePath;
  ...
  if (process.platform === "win32") {
    ...
    return preferExisting(path.join(appData, "airoute"), path.join(appData, "omniroute"));
  }

  const xdgConfigHome = process.env.XDG_CONFIG_HOME;
  if (typeof xdgConfigHome === "string" && xdgConfigHome.trim().length > 0) {
    const xdg = path.resolve(xdgConfigHome.trim());
    return preferExisting(path.join(xdg, "airoute"), path.join(xdg, "omniroute"));
  }

  return preferExisting(airouteDir, omnirouteDir);
}
