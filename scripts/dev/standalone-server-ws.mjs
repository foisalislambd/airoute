...
// and AIROUTE_TLS_KEY (or legacy OMNIROUTE_TLS_*) point at readable files we
// terminate TLS on the same listener Next binds to (so WS `upgrade` / request
// wrappers keep working over TLS). Absent or misconfigured → null → identical
// plain-HTTP behavior as before.
const tlsOptions = resolveTlsOptions(process.env);
process.env.AIROUTE_INTERNAL_SCHEME = tlsOptions ? "https" : "http";
process.env.OMNIROUTE_INTERNAL_SCHEME = process.env.AIROUTE_INTERNAL_SCHEME;
if (tlsOptions) {
  console.log(`[airoute][tls] HTTPS enabled — terminating TLS with cert=${tlsOptions.certPath}`);
}

process.env.AIROUTE_WS_BRIDGE_SECRET ||= process.env.OMNIROUTE_WS_BRIDGE_SECRET || randomUUID();
process.env.OMNIROUTE_WS_BRIDGE_SECRET = process.env.AIROUTE_WS_BRIDGE_SECRET;
