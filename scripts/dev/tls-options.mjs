 ...
 *
 * Reads `AIROUTE_TLS_CERT` / `AIROUTE_TLS_KEY` (filesystem paths), with
 * `OMNIROUTE_TLS_*` as legacy fallbacks. Returns `{ cert, key }` (file contents)
 * only when BOTH are provided and readable. Otherwise returns `null` so the
 * caller serves plain HTTP — never throwing.
 *
...
export function resolveTlsOptions(
  ...
  const rawCert = env?.AIROUTE_TLS_CERT || env?.OMNIROUTE_TLS_CERT;
  const rawKey = env?.AIROUTE_TLS_KEY || env?.OMNIROUTE_TLS_KEY;
  const certPath = typeof rawCert === "string" ? rawCert.trim() : "";
  const keyPath = typeof rawKey === "string" ? rawKey.trim() : "";
