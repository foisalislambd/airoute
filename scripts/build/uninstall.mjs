...
try {
  ...
  execSync("pm2 delete airoute 2>/dev/null || pm2 delete omniroute 2>/dev/null", {
    stdio: "ignore",
  });
} catch {
...
const defaultDataDir = (() => {
  const airoute = path.join(home, ".airoute");
  const omniroute = path.join(home, ".omniroute");
  if (fs.existsSync(airoute)) return airoute;
  if (fs.existsSync(omniroute)) return omniroute;
  return airoute;
})();
