...
function resolveDataDir(overridePath, env = process.env) {
  ...
  const preferExisting = (airoutePath, omniPath) => {
    try {
      if (existsSync(airoutePath)) return airoutePath;
    } catch {
      /* ignore */
    }
    try {
      if (existsSync(omniPath)) return omniPath;
    } catch {
      /* ignore */
    }
    return airoutePath;
  ...
  if (process.platform === "win32") {
    ...
    return preferExisting(join(appData, "airoute"), join(appData, "omniroute"));
  }

  const xdg = env.XDG_CONFIG_HOME?.trim();
  if (xdg) {
    return preferExisting(join(resolve(xdg), "airoute"), join(resolve(xdg), "omniroute"));
  }

  return preferExisting(join(home, ".airoute"), join(home, ".omniroute"));
}
        ...
        if (row) {
          ...
          if (ciphertext?.startsWith("enc:v1:")) {
            ...
            if (parts.length >= 5) {
              ...
              // AIRoute static salt (primary encrypt path)
              const airouteSalt = "airoute-field-encryption-v1";
              const airouteKey = scryptSync(merged.STORAGE_ENCRYPTION_KEY, airouteSalt, 32);

              // Legacy OmniRoute static salt
              const legacySalt = "omniroute-field-encryption-v1";
              const legacyKey = scryptSync(merged.STORAGE_ENCRYPTION_KEY, legacySalt, 32);

              let keyMatched = false;
              for (const candidate of [airouteKey, dynamicKey, legacyKey]) {
                try {
                  tryDecrypt(candidate);
                  keyMatched = true;
                  break;
                } catch {
                  // try next derivation
